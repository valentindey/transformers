...
- In addition to `input_ids`, :meth:`~transformer.LayoutLMModel.forward` also expects the input :obj:`bbox`, which are
  ...
  parameter for the tokenizer class with :obj:`target_height_and_width`, default is 1000x1000). When encoding the input,
  you may provide an :obj:`orig_height_and_width` parameter correspond to the width and height of the original document
  in which the token occurs.
  Width and height can be obtained using the Python Image Library (PIL) library for example, as follows:
