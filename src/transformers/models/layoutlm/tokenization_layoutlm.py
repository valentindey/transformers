...
from ...tokenization_utils_base import PaddingStrategy, EncodedInput, BatchEncoding, TextInput, PreTokenizedInput, \
    TruncationStrategy, ENCODE_KWARGS_DOCSTRING, ENCODE_PLUS_ADDITIONAL_KWARGS_DOCSTRING, EncodedInputPair, \
    PreTokenizedInputPair, TextInputPair
...
TEXT_AND_BOX_ARGS_DOCSTRING = r"""
            ...
            bbox (:obj:`BoundingBox`, :obj:`List[BoundingBox]`, :obj:`List[List[BoundingBox]]`):
                The bounding boxes associated with the given :obj:`text`
                A bounding box is defined by (top, left, right, bottom) coordinates
                Floating point coordinates will be rounded to integer values as expected by the LayoutLM model
                The values are optionally normalized to a page's width and heigth, see parameter :obj:`orig_width_and_height`
            text_pair (:obj:`str`, :obj:`List[str]`, :obj:`List[List[str]]`):
                The sequence or batch of sequences to be encoded. Each sequence can be a string or a list of strings
                (pretokenized string). If the sequences are provided as list of strings (pretokenized), you must set
                :obj:`is_split_into_words=True` (to lift the ambiguity with a batch of sequences).
            bbox_pair (:obj:`BoundingBox`, :obj:`List[BoundingBox]`, :obj:`List[List[BoundingBox]]`):
                Additional bounding boxes associated with the given :obj:`text_pair`
                A bounding box is defined by (top, left, right, bottom) coordinates
                Floating point coordinates will be rounded to integer values as expected by the LayoutLM model
                The values are optionally normalized to a page's width and heigth, see parameter :obj:`orig_width_and_height`
            orig_width_and_height (:obj:`Tuple[float, float]`, :obj:`List[Tuple[float, float]]` `optional`):
                size of original document page from which bounding boxes are taken
                if this argument is passed, the bounding box coordinates are normalized accordingly
                in case of batch inputs, a list of size tuples may be passed, otherwise the same size is applied for
...
class LayoutLMTokenizer(BertTokenizer):
    ...
    It additionally takes care of handling bounding boxes by repeating a bounding box associated with the corresponding
    text. Additional default boxes are defined for [SEP], [PAD] and [CLS] tokens and added correspondingly.
    Bounding box coordinates will be rounded to integer values and may optionally be normalized to a target width and
    height.
    ...
    Args:
        ...
        target_height_and_width (:obj:`Tuple[int, int]`, defaults to :obj:`(1000, 1000)`):
            the target width and height of bounding boxes
            only has an effect when the :obj:`orig_height_and_width` argument is passed to the tokenization methods
    """
    ...
    def __call__(
        ...
        sequences.
        Identical to :obj:`PreTrainedTokenizerBase.__call__` with additional handling of bounding boxes.
        If a text is split further the corresponding bounding boxes will be repeated that many times.
        The bounding boxes may optionally be normalized to the given width and height, according to the parameters of
    ...
    def _check_num_samples(self, text, bbox):
        ...
        if isinstance(text, (list, tuple)):
            if isinstance(text[0], (list, tuple)):
                ...
                assert isinstance(bbox[0], tuple) and (isinstance(bbox[0][0], (float, int))), (
    ...
    def encode(
        ...
        **kwargs
        ...
        Adds additional bounding boxes for split words.
        """
    ...
    def batch_encode_plus(
            ...
            batch_bbox_or_bbox_pairs: Union[
                List[BoundingBox],
                List[BoundingBoxList],
                List[Tuple[BoundingBox, BoundingBox]]
            ],
    ...
    def _encode_plus(
        ...
        second_ids, second_bbox = get_input_ids_and_extend_bbox(text_pair, bbox_pair) if text_pair is not None else (None, None)
    ...
    def _batch_encode_plus(
        ...
        for ids_or_pair_ids, bbox_or_pair_bbox in zip(batch_text_or_text_pairs, batch_bbox_or_bbox_pairs):
            ...
            second_ids, second_bbox = get_input_ids_and_extend_bbox(pair_ids, pair_boxes) if pair_ids is not None else (None, None)
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        they can be used by the model. It adds special tokens and boxes, truncates sequences if overflowing while taking
        into account the special tokens and manages a moving window (with user defined stride) for overflowing tokens
        """
    ...
    def _batch_prepare_for_model(
        ...
        taking into account the special tokens and manages a moving window (with user defined stride) for overflowing tokens
        ...
        batch_outputs = {}
        for i, ((first_ids, second_ids), (first_boxes, second_boxes)) in enumerate(zip(batch_ids_pairs, batch_box_pairs)):
    ...
    def _pad(
        ...
        Identical to :obj:`PreTrainedTokenizerBase._pad` but additionally handles bounding box inputs, padding with
        :obj:`self.pad_box`
        ...
        if len(input_ids) != len(bbox):
            raise ValueError("encountered differently sized input_ids and bbox lists, "
